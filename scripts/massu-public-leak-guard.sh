...
ALLOWED_PATTERNS=(
  ...
  '^\.github/rulesets/'
  # NOTE: `.github/dependabot.yml` is deliberately NOT allowlisted (added
  # 2026-08-07, removed the same day). The mirror gets no Dependabot config —
  # its PRs are unmergeable by construction, see scripts/PUBLIC_MANIFEST.md
  # "NOT Synced (Private)". Leaving it OFF the allowlist means a hand-placed
  # copy is BLOCKED rather than silently published and then wiped by the next
  # sync — which is the behaviour we want.
)
