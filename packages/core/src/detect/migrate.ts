...
export function migrateV1ToV2(
  ...
  // (e.g., example-project's `framework.{python, rust, swift, typescript}` language sub-blocks).
  ...
  // (e.g., example-project's 19 custom `paths.*` entries like adr, plans, monorepo_root).
  ...
  // (e.g., example-project's `project.description`) survive the migration.
  ...
  // like `services`, `workflow`, `north_stars` (example-project) now pass through.
  //
