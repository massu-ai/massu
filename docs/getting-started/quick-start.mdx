...
- Initialization complete (`npx massu init`) -- this also installs 43 slash commands into `.claude/commands/`
- All checks passing (`npx massu doctor`)

After init completes, all 43 slash commands (like `/massu-plan`, `/massu-loop`, `/massu-commit`) are available immediately in Claude Code -- no extra setup needed.
...
- **[Hooks](/docs/hooks)**: Understand the 15-hook lifecycle
