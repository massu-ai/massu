...
curl -H "Authorization: Bearer ms_live_XXXXXXXX_your_secret_key" \
  https://api.massu.ai/v1/sessions
```
...
is available via `/v1/sessions/[id]` which includes the
...
> **Note**: budget data is included in the `/v1/cost` summary payload
> via the `current_budget` field — no separate `/v1/cost/budget`
...
> `/v1/security/alerts` and `/v1/security/score` were removed —
...
> P-M-043 (plan-stage-d-medium-sweep): prior doc entries for
> `/v1/team/members` and `/v1/team/activity` were removed —
...
> `/v1/risk/prs` which never existed as a route. The list payload
> below IS the default `/v1/risk` response shape.
