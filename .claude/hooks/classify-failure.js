...
init_config();
init_memory_db();
import { existsSync as existsSync7, readFileSync as readFileSync4, readdirSync as readdirSync2, unlinkSync as unlinkSync2, writeFileSync as writeFileSync2 } from "fs";
...
function getDedupeMarkerPath(sessionId, filePath) {
  ...
  return join4(tmpdir(), `massu-classify-${day}-${sessionId.slice(0, 8)}-${hash}`);
}
...
function readFailureContextFiles() {
  ...
  try {
    ...
    for (const file of files) {
      try {
        context += " " + readFileSync4(join4(dir, file), "utf-8");
      } catch {
      }
    }
  } catch {
...
async function main() {
  try {
    ...
    if (existsSync7(dedupeMarker)) {
      process.exit(0);
      return;
    }
    try {
      writeFileSync2(dedupeMarker, "1");
    } catch {
    }
