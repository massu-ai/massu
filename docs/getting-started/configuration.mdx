...
cloud:
  enabled: true
  # apiKey is optional here — the git-safe paths below are preferred
  endpoint: "https://api.massu.ai/v1"   # optional; this is the default
  sync:
    memory: true
    analytics: true
    audit: true
...
| `apiKey` | string | - | Your Massu AI API key (starts with `ms_live_`). Optional — see the key-precedence note below |
| `endpoint` | string | `https://api.massu.ai/v1` | Cloud API endpoint. Leave unset to use the default; override only for self-hosted / enterprise deployments |
...
**Where to put your key (precedence).** Your API key is resolved from the first available source in this fixed order:

1. `cloud.apiKey` in this file
2. the `MASSU_API_KEY` environment variable
3. the `~/.massu/credentials` user-level file (written by `massu login`)

The **recommended, git-safe** path is `massu login` (stores the key in `~/.massu/credentials`, outside every repository) or the `MASSU_API_KEY` environment variable — both keep the secret out of your committed config. Only set `cloud.apiKey` directly if you understand the risk of a key sitting in a file that may be committed. Note the config loader does **not** expand shell-style `${VAR}` placeholders, so writing the env var name in dollar-brace form as the `apiKey` value stores that literal text rather than your key — use the env var or `massu login` instead.
...
|----------|-------------|
| `MASSU_API_KEY` | Your Massu AI cloud API key (git-safe alternative to `cloud.apiKey`). Takes precedence over the `~/.massu/credentials` file but not over an explicit `cloud.apiKey` |
| `MASSU_CLOUD_ENDPOINT` | Override the cloud API endpoint (default `https://api.massu.ai/v1`); for self-hosted / enterprise deployments |
