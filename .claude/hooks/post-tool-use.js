...
import { resolve as resolve6, dirname as dirname6, join as join4 } from "path";
...
function resolveImportPath(specifier, fromFile) {
  ...
  if (specifier.startsWith("@/")) {
    ...
    basePath = resolve6(dirname6(fromFile), specifier);
  }
...
// src/hooks/post-tool-use.ts
init_config();
import { readFileSync as readFileSync8, writeFileSync as writeFileSync2, existsSync as existsSync11, mkdirSync as mkdirSync5, statSync as statSync3 } from "fs";
import { join as join5, basename as pathBasename2, dirname as pathDirname, resolve as pathResolve } from "path";
...
import { readFileSync as readFileSync7, existsSync as existsSync10, readdirSync as readdirSync2 } from "fs";
init_memory_db();
...
function ingestMemoryFile(db, sessionId, filePath) {
  if (!existsSync10(filePath)) return "skipped";
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          const dir = join5(projectRoot, ".massu", "rule-candidates");
          if (!existsSync11(dir)) mkdirSync5(dir, { recursive: true });
          const logPath = join5(dir, ".cr53-increment-failures.jsonl");
          const pre = existsSync11(logPath) ? readFileSync8(logPath, "utf-8") : "";
...
function readConventions(cwd) {
  try {
    ...
    const configPath = join5(projectRoot, "massu.config.yaml");
    if (!existsSync11(configPath)) return _conventionDefaults;
