...
| **Auto-Learning / Rule Promotion** | `/massu-rule` (candidate detection + promotion) | Turn your corrections into enforced rules — the hook detects correction-shaped prompts and `/massu-rule approve` promotes them to pattern-scanner checks, CR rows, or `corrections.md` entries. Pro = local to your machine. |
...
On the Team tier, auto-learning is designed to become **shared** — a promoted rule propagates across the team rather than staying local to one machine. Shared rule propagation is on the roadmap; today auto-learning is local to your machine on both Pro and Team.
