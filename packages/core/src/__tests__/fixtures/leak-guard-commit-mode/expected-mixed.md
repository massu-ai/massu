...
The trigger word appears in the middle of the file, surrounded by prose that does NOT match the leak pattern, to ensure the scanner doesn't short-circuit on the first non-matching paragraph: the word example-project is present here as a regression test.
