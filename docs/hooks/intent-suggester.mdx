...
|----------|------------------|
| `fix bug`, `debug`, `investigate error`, `trace` | `/massu-debug` |
| `add test`, `test failing`, `test coverage`, `generate test` | `/massu-test` |
| `refactor`, `restructure`, `rename`, `reorganize` | `/massu-refactor` |
| `security`, `vulnerability`, `owasp`, `rls`, `secrets` | `/massu-security-scan` |
| `deploy`, `release`, `version bump`, `tag` | `/massu-release` |
| `review`, `code review`, `pr review` | `/massu-review` |
| `migrate`, `migration`, `schema change`, `alter table` | `/massu-migrate` |
| `performance`, `slow`, `bundle size`, `optimize` | `/massu-perf` |
| `hotfix`, `urgent fix`, `production bug` | `/massu-hotfix` |
| `plan`, `implement feature`, `new feature` | `/massu-create-plan` |
| `commit`, `ready to commit` | `/massu-commit` |
| `push`, `ready to push` | `/massu-push` |
| `cleanup`, `dead code`, `unused imports`, `orphaned` | `/massu-cleanup` |
| `documentation`, `jsdoc`, `generate docs`, `readme` | `/massu-doc-gen` |
| `onboard`, `new team member`, `getting started guide` | `/massu-onboard` |
| `retrospective`, `sprint review`, `what happened` | `/massu-retrospective` |
| `accessibility`, `wcag`, `a11y`, `screen reader` | `/massu-accessibility` |
| `estimate`, `effort`, `story points`, `complexity` | `/massu-estimate` |
| `benchmark`, `baseline`, `performance test` | `/massu-benchmark` |
| `dependencies`, `outdated`, `audit deps` | `/massu-audit-deps` |
| `health`, `project status`, `what is broken` | `/massu-status` |
| `changelog`, `release notes` | `/massu-changelog` |
