...
description: "15 MCP lifecycle hooks plus 11 shell hooks that capture context, enforce rules, auto-learn from bug fixes, and preserve memory without any manual effort"
order: 0
---

# Lifecycle Hooks

Massu AI includes 15 MCP lifecycle hooks and 11 shell utility hooks that run automatically during Claude Code sessions. Together they capture observations, inject context, enforce rules, detect and learn from bug fixes, filter output, and preserve state -- all without any manual effort from the developer.
...
| [fix-detector](/docs/hooks/fix-detector) | After file edit | Detect bug fixes via git diff | After Edit/Write on tracked files |
| [auto-learning-pipeline](/docs/hooks/auto-learning-pipeline) | Session ends | Enforce pipeline completion | Session stop |
| [incident-pipeline](/docs/hooks/incident-pipeline) | After file write | Trigger rule derivation from incidents | After Write to incident files |
| [rule-enforcement-pipeline](/docs/hooks/rule-enforcement-pipeline) | After file write | Trigger enforcement from rules | After Write to rule files |
...
[fix-detector hook] --> Detects bug fixes via git diff
[incident-pipeline hook] --> Triggers rule derivation on incident writes
[rule-enforcement-pipeline hook] --> Triggers enforcement on rule writes
...
[auto-learning-pipeline hook] --> Enforces fix→incident→rule→enforcement completion
```
...
- [fix-detector](/docs/hooks/fix-detector) -- Bug fix detection via git diff heuristics
- [auto-learning-pipeline](/docs/hooks/auto-learning-pipeline) -- Pipeline completion enforcement at session end
- [incident-pipeline](/docs/hooks/incident-pipeline) -- Rule derivation trigger on incident report writes
- [rule-enforcement-pipeline](/docs/hooks/rule-enforcement-pipeline) -- Enforcement placement trigger on rule writes

## The Auto-Learning Pipeline

The 4 auto-learning hooks form a closed-loop system that turns every bug fix into an enforced project rule:

```
Bug Fix Detected ──► Incident Report ──► Prevention Rule ──► Enforcement Hook
 (fix-detector)    (incident-pipeline)  (rule-enforcement-   (project hooks
                                          pipeline)           & config)
```

1. **fix-detector** monitors file edits and analyzes git diffs for fix signals (removed broken code, added error handling, auth fixes, nil handling, concurrency fixes, etc.)
2. **incident-pipeline** fires when an incident report is written to `docs/incidents/`, prompting Claude to derive a prevention rule
3. **rule-enforcement-pipeline** fires when a prevention rule (`memory/feedback_*.md`) is written, prompting Claude to place enforcement in project hooks or config
4. **auto-learning-pipeline** runs at session end and blocks completion if fixes were detected but the pipeline was not finished

Configure via the `autoLearning` section in `massu.config.yaml`:
...
autoLearning:
  ...
  pipeline:
    ...
    requirePreventionRule: true
    requireEnforcement: true
```

## Shell Hooks (11)

In addition to the 15 MCP lifecycle hooks, Massu AI includes 11 shell utility hooks located in `scripts/hooks/`. These hooks are invoked at specific points to enforce policies, filter output, and automate routine tasks.
