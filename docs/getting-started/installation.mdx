...
Massu AI installs with one command. It automatically detects your framework, creates your configuration, registers the MCP server, installs all 15 lifecycle hooks, and deploys 43 slash commands.
...
========================
  ...
  Installed 15 hooks in .claude/settings.local.json
  Installed 43 slash commands in .claude/commands/
...
4. **Hook Installation** -- Configures all 15 lifecycle hooks in `.claude/settings.local.json`
5. **Slash Command Installation** -- Deploys 43 workflow commands into `.claude/commands/`
...
15 hooks monitoring this session. 43 slash commands available.
...
=====================
  ...
  ✓ Hooks Config       15 hooks configured
  ✓ Hook Files         15/15 compiled hooks present
  ✓ Slash Commands     31/43 commands installed
...
| `fix-detector` | PostToolUse | Detects bug fixes via git diff heuristics |
| `auto-learning-pipeline` | Stop | Enforces fix→incident→rule→enforcement pipeline completion |
| `incident-pipeline` | PostToolUse | Triggers rule derivation when incident reports are written |
| `rule-enforcement-pipeline` | PostToolUse | Triggers enforcement placement when prevention rules are written |
...
your-project/
  ...
  .claude/
    ...
    commands/                    # 43 slash commands (auto-generated)
      massu-plan.md
      massu-loop.md
      massu-commit.md
...
Re-deploys all 43 slash commands into `.claude/commands/` without running the full init process. Useful after updating `@massu/core` to get the latest command definitions.
