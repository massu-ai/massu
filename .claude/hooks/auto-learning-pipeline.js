...
import { existsSync as existsSync8, readFileSync as readFileSync5, unlinkSync as unlinkSync2, readdirSync as readdirSync2, statSync as statSync2 } from "fs";
...
function getSessionFlagPath(sessionId) {
  return join5(tmpdir(), "massu-auto-learning", `fixes-${sessionId.slice(0, 12)}.jsonl`);
}
async function main() {
  try {
    ...
    if (existsSync8(flagPath)) {
      try {
        sessionFixes = readFileSync5(flagPath, "utf-8").split("\n").filter(Boolean).map((line) => JSON.parse(line));
      } catch {
      }
    }
