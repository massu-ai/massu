...
Full project setup in one command. Detects your framework, generates configuration, registers the MCP server, installs all 15 lifecycle hooks, and deploys 43 slash commands.
...
| `.claude/commands/*.md` | 43 slash command definitions |
...
| Hook Files | All 15 compiled hook files exist |
| Slash Commands | `.claude/commands/` has 43 command files |
...
Install or update all 15 lifecycle hooks without running the full init process.
...
Install or update all 43 slash commands without running the full init process.
