...
init_config();
import { existsSync as existsSync7, readFileSync as readFileSync4, readdirSync as readdirSync2 } from "fs";
...
async function main() {
  try {
    ...
    if (existsSync7(enforcementDirAbs)) {
      const hookFiles = readdirSync2(enforcementDirAbs).filter((f) => f.endsWith(".sh") || f.endsWith(".ts") || f.endsWith(".js"));
      for (const hookFile of hookFiles) {
        try {
          const hookContent = readFileSync4(resolve5(enforcementDirAbs, hookFile), "utf-8");
