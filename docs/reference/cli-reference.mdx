---
title: "CLI Reference"
description: "Complete reference for all Massu CLI commands — init, doctor, install-hooks, install-commands, validate-config"
order: 2
---
...
=====================
  ...
  ✓ Slash Commands     51/59 commands installed
  ✓ Native Modules     better-sqlite3 loads correctly
...
Install or update all project assets without running the full init process. Syncs 59 slash commands, 11 agent definitions, 9 pattern files, 3 protocols, and 8 reference documents.
...
- After updating `@massu/core` to a new version (to get the latest command and asset definitions)
- If command or asset files were accidentally deleted from `.claude/`
- To re-install all assets without regenerating config, MCP registration, or hooks

### Output

```
Massu AI - Install Project Assets
==================================

  slash commands: 12 new, 47 current (59 total)
  agent definitions: 11 current (11 total)
  pattern files: 9 current (9 total)
  protocol files: 3 current (3 total)
  reference files: 8 current (8 total)

  90 total files synced to .claude/

  Restart your Claude Code session to use them.
```
