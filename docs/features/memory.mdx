...
### `massu_memory_backfill`

**What it does:** Scans all `memory/*.md` files in your project's memory directory and bulk-ingests them into the database. Parses YAML frontmatter (name, description, type, confidence) and deduplicates by title. Run after `massu init` or to recover from database loss.
...
Successfully processed 58 of 58 memory files.
```

**Parameters:** None -- scans the memory directory automatically.
...
- **`post-tool-use` hook**: Classifies every tool call into observation types (file_change, bugfix, feature, decision, failed_attempt, etc.). Also **auto-ingests** `memory/*.md` files when they are written or edited -- parsing YAML frontmatter and storing the content in the observations table for MCP tool queries.
- **`user-prompt` hook**: Captures every user prompt for full-text search. Also **nags** when significant work is detected but no memory files have been written.
