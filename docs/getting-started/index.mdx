...
Massu AI is an **AI Engineering Governance Platform** that transforms how your team works with AI coding assistants. It is an MCP server and Claude Code plugin that provides cross-session memory, a structured knowledge system, automated quality tracking, compliance audit trails, and 84 specialized tools -- all running locally with zero cloud dependencies.
...
- **84 MCP tools** for code intelligence, security scoring, cost tracking, knowledge search, and more
...
### 84 Purpose-Built Tools

Massu AI provides 84 MCP tools organized across eighteen categories. Free-tier tools (12) are available to everyone; additional tools unlock with [Pro, Team, and Enterprise tiers](/docs/reference/license-tiers):
...
When you run `npx massu init`, Massu AI automatically configures the memory system, installs all 11 lifecycle hooks, and deploys 59 slash commands into `.claude/commands/`. No additional setup is required. The `session-start` hook begins injecting context from your very first session. To check memory is working:
...
Your Prompt --> Claude Code --> Massu AI Hooks --> Memory DB + Knowledge DB
                    |                                          |
                    v                                          v
              84 MCP Tools <-- Context Injection <-- Past Sessions + Knowledge
                    |
                    v
...
3. **Claude Code calls tools** -- Massu AI's 84 MCP tools provide code intelligence, memory search, knowledge lookup, quality scoring, and more
