...
| [pre-tool-use-gate](/docs/hooks/pre-tool-use-gate) | Before tool execution | Consolidated security-gate + pre-delete-check (single-spawn) | Before every tool |
...
| [pre-delete-check](/docs/hooks/pre-delete-check) | Before file deletion | Feature impact check (back-compat; folded into pre-tool-use-gate) | Before rm/delete operations |
| [post-edit-context](/docs/hooks/post-edit-context) | After file edit | Surface applicable rules | After Edit/Write on src/ files |
| [security-gate](/docs/hooks/security-gate) | Before tool execution | Validate security policies (back-compat; folded into pre-tool-use-gate) | Before dangerous operations |
...
| [fix-detector](/docs/hooks/fix-detector) | After Edit/Write to source | Detect bug-fix shape via git diff; arm auto-learning pipeline | After every code edit |
| [classify-failure](/docs/hooks/classify-failure) | After Edit/Write to source | Score detected fix against known failure classes (KNOWN / SIMILAR / NEW) | After every code edit |
| [incident-pipeline](/docs/hooks/incident-pipeline) | After Write to docs/incidents/*.md | Trigger rule derivation from new incident report | When incident written | <!-- leak-guard-allow: documents the configurable default incident path, not an internal reference -->
| [rule-enforcement-pipeline](/docs/hooks/rule-enforcement-pipeline) | After Write to memory/feedback_*.md | Trigger enforcement placement for new rule | When feedback rule written |
| [auto-learning-pipeline](/docs/hooks/auto-learning-pipeline) | Session ends with armed fix | Forcing function: block session-end until incident → rule → enforcement complete | Stop hook with pending fix |
...
- [pre-tool-use-gate](/docs/hooks/pre-tool-use-gate) -- Consolidated PreToolUse single-spawn check (security + delete-protection)
...
- [pre-delete-check](/docs/hooks/pre-delete-check) -- Deletion protection via feature impact (back-compat; folded into pre-tool-use-gate)
- [post-edit-context](/docs/hooks/post-edit-context) -- Rule surfacing after file edits
- [security-gate](/docs/hooks/security-gate) -- Security policy validation before tool execution (back-compat; folded into pre-tool-use-gate)
...
- [fix-detector](/docs/hooks/fix-detector) -- Fix-shape detection via git diff (arms auto-learning pipeline)
- [classify-failure](/docs/hooks/classify-failure) -- Scores detected fix against known failure classes
- [incident-pipeline](/docs/hooks/incident-pipeline) -- Triggers rule derivation when incident reports are written
- [rule-enforcement-pipeline](/docs/hooks/rule-enforcement-pipeline) -- Triggers enforcement placement when feedback rules are written
- [auto-learning-pipeline](/docs/hooks/auto-learning-pipeline) -- Stop-hook forcing function for the incident → rule → enforcement chain
