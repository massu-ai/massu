...
| Tier | What You Get |
|------|-------------|
| **Free** | 13 tools -- core navigation (`sync`, `context`, `impact`, `domains`, `schema`, `trpc_map`, `coupling_check`), basic memory (`memory_search`, `memory_ingest`, `memory_backfill`), regression detection (`regression_risk`, `feature_health`), and `license_status` |
| **Pro** | All Free tools plus knowledge search and indexing (12 tools), advanced memory (5 tools), quality analytics (3 tools), cost tracking (3 tools), prompt analysis (2 tools), validation (2 tools), ADR management (3 tools), observability (4 tools), documentation (2 tools), and Python intelligence (8 tools) |
