...
import { existsSync as existsSync7, appendFileSync as appendFileSync2, mkdirSync as mkdirSync5, readFileSync as readFileSync4 } from "fs";
...
function getSessionFlagPath(sessionId) {
  const dir = join4(tmpdir(), "massu-auto-learning");
  if (!existsSync7(dir)) {
    mkdirSync5(dir, { recursive: true });
  }
  return join4(dir, `fixes-${sessionId.slice(0, 12)}.jsonl`);
}
function _stateDir() {
  const dir = join4(tmpdir(), "massu-fix-detector-state");
  if (!existsSync7(dir)) {
    mkdirSync5(dir, { recursive: true });
  }
  return dir;
...
function isGitWorkTreeFast(cwd) {
  try {
    const cachePath = join4(_stateDir(), `worktree-${_cwdHash(cwd)}.json`);
    if (existsSync7(cachePath)) {
      const cached = JSON.parse(readFileSync4(cachePath, "utf-8"));
...
function _disabledFlagPath(sessionId) {
  return join4(_stateDir(), `disabled-${sessionId.slice(0, 12)}.flag`);
}
function isSessionAutoDisabled(sessionId) {
  try {
    return existsSync7(_disabledFlagPath(sessionId));
  } catch {
    return false;
  }
}
...
async function main() {
  try {
    ...
    appendFileSync2(flagPath, JSON.stringify(signal) + "\n");
    const lines = readFileSync4(flagPath, "utf-8").split("\n").filter(Boolean);
    if (lines.length === 1) {
      writeHookMessage(
