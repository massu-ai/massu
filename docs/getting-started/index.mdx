...
Massu AI provides 72 MCP tools organized across eighteen categories. Free-tier tools (12) are available to everyone; additional tools unlock with [Pro, Team, and Enterprise tiers](/docs/reference/license-tiers):
...
When you run `npx massu init`, Massu AI automatically configures the memory system, installs all 11 lifecycle hooks, and deploys 43 slash commands into `.claude/commands/`. No additional setup is required. The `session-start` hook begins injecting context from your very first session. To check memory is working:
