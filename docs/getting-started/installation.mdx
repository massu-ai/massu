...
Massu AI installs with one command. It automatically detects your framework, creates your configuration, registers the MCP server, installs all 16 lifecycle hooks, and deploys 59 slash commands.
...
16 hooks monitoring this session. 59 slash commands available.
...
=====================
  ...
  ✓ Hook Files         16/16 compiled hooks present
...
| `fix-detector` | PostToolUse | Detects bug fixes on file changes; populates the auto-learning queue |
| `classify-failure` | PostToolUse | Categorizes test/build failures so similar issues can be matched later |
| `incident-pipeline` | PostToolUse | Auto-creates incident reports for new failure classes |
| `rule-enforcement-pipeline` | PostToolUse | Surfaces enforcement layer recommendations for new prevention rules |
| `auto-learning-pipeline` | Stop | At session end, ensures every detected fix has incident + rule + enforcement coverage |
...
your-project/
  ...
  .claude/
    ...
    commands/                    # 59 slash commands (auto-generated)
      massu-plan.md
      massu-loop.md
      massu-commit.md
...
Re-deploys all 59 slash commands into `.claude/commands/` without running the full init process. Useful after updating `@massu/core` to get the latest command definitions. Also seeds `mcp__massu__*` into `.claude/settings.local.json`'s `permissions.allow` so MCP tool calls skip the per-tool permission dialog — pass `--skip-permissions` to opt out, or run `npx massu permissions verify` to audit the allowlist state.
