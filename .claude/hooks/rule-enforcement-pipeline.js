...
// src/lib/fileLock.ts
import { mkdirSync as mkdirSync2, readFileSync as readFileSync3, rmSync as rmSync2, writeFileSync as writeFileSync2 } from "fs";
import { dirname as dirname2 } from "path";
import * as lockfile from "proper-lockfile";
function readLockHolderPid(lockPath) {
  try {
    const raw = readFileSync3(`${lockPath}.pid`, "utf-8").trim();
...
import { accessSync, appendFileSync, chmodSync as chmodSync2, constants as fsConstants, existsSync as existsSync3, mkdirSync as mkdirSync3 } from "fs";
...
function attemptNativeHeal(err) {
  ...
  let pkgDir;
  try {
    pkgDir = dirname3(req.resolve("better-sqlite3/package.json"));
  } catch {
...
import { closeSync, existsSync as existsSync4, mkdtempSync, openSync, readFileSync as readFileSync4, rmSync as rmSync3, fsyncSync } from "fs";
...
init_config();
import { existsSync as existsSync9, readFileSync as readFileSync6, readdirSync as readdirSync2 } from "fs";
...
async function main() {
  try {
    ...
    const enforcementDirAbs = resolve5(root, enforcementDir);
    let hasEnforcement = false;
    if (existsSync9(enforcementDirAbs)) {
      const hookFiles = readdirSync2(enforcementDirAbs).filter((f) => f.endsWith(".sh") || f.endsWith(".ts") || f.endsWith(".js"));
      for (const hookFile of hookFiles) {
        try {
          const hookContent = readFileSync6(resolve5(enforcementDirAbs, hookFile), "utf-8");
    ...
    recordHookFailure("rule-enforcement-pipeline", err);
  }
  process.exit(0);
}
function readStdin() {
