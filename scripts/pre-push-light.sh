...
#
# 21 labels [0/20]..[20/20] (plan-2026-05-18-pre-push-ci-parity, P2-001..P2-006;
# step [16/20] added by plan-2026-06-01-claude-md-size-compliance;
# steps [17/20]+[18/20] added by plan-2026-06-03-website-lib-test-coverage).
# Plan-Token Changelog Currency REMOVED (P2-006 iter-5): absorbed by
# `changelog-parse.test.ts` EXPECTED_COUNT drift-guard run via [6/20] Tests.
#
#  [0/20] Clean-state simulation (opt-in via MASSU_PREPUSH_CLEAN=1)
#  [1/20] Node version pre-flight
#  [2/20] Pattern Scanner
#  [3/20] Security Scanner
#  [4/20] Hook Build
#  [5/20] TypeScript
#  [6/20] Tests
#  [7/20] Plan Status Validator
#  [8/20] Plan Commit Drift
#  [9/20] Deploy Staleness
# [10/20] Dist-Tag Pre-Release
# [11/20] Public Content Leak Guard
# [12/20] Workspace Build Freshness                (NEW — P2-005)
# [13/20] Sync Check (public-mirror)               (NEW — P2-002; auto-gated on packages/, scripts/, etc.)
# [14/20] Tarball E2E (quick)                      (NEW — P2-003)
# [15/20] Config Drift                             (NEW — P2-004)
# [16/20] CLAUDE.md Size                            (NEW — plan-2026-06-01-claude-md-size-compliance)
# [17/20] Coverage Gate (real v8 line %)            (NEW — plan-2026-06-03-website-lib-test-coverage)
# [18/20] Plan-Token Changelog Coverage             (NEW — plan-2026-06-03-website-lib-test-coverage; CR-50 parity mirror of CI)
# [19/20] Incident Coverage (CR-62)                 (NEW — incident 2026-07-12-memory-subsystem-silent-data-loss-cluster)
# [20/20] Claim Ledger (CR-63)                      (NEW — universal/capability claims need EXECUTED evidence)
#
# Bypass env vars (all emit `[pre-push-light] BYPASS/OPTIN via ...` to stderr for audit-trail):
#   MASSU_PREPUSH_CLEAN=1              — opt-in step [0/20] clean-state simulation
#   MASSU_SKIP_NEW_STEPS=1             — skip [12/20]..[15/20] (ceremony bootstrap)
#   MASSU_PREPUSH_SYNC_CHECK=0         — skip [13/20] sync-check (default ON, auto-gated)
#   MASSU_SKIP_DEPLOY_STALENESS_CHECK=1 — skip [9/20] deploy staleness (pre-existing CR-48 bypass)
#
...
if [ "${MASSU_SKIP_NEW_STEPS:-0}" = "1" ]; then
  echo "[pre-push-light] BYPASS via MASSU_SKIP_NEW_STEPS=1 at $(date -u +%Y-%m-%dT%H:%M:%SZ) — skipping steps [12/20]..[15/20] (Workspace Build Freshness, Sync Check, Tarball E2E, Config Drift) — logged for audit-trail" >&2
fi
if [ "${MASSU_PREPUSH_CLEAN:-0}" = "1" ]; then
  echo "[pre-push-light] OPTIN via MASSU_PREPUSH_CLEAN=1 at $(date -u +%Y-%m-%dT%H:%M:%SZ) — Step [0/20] Clean-state simulation: ENABLED — logged for audit-trail" >&2
fi
if [ "${MASSU_PREPUSH_SYNC_CHECK:-1}" = "0" ]; then
  echo "[pre-push-light] BYPASS via MASSU_PREPUSH_SYNC_CHECK=0 at $(date -u +%Y-%m-%dT%H:%M:%SZ) — skipping step [13/20] Sync Check — logged for audit-trail" >&2
fi

# --- Filesystem-derived workspace package list (single SoT for [0/20] +
#     [12/20]). Future adapter additions/removals don't need to update
...
# [0/20] Clean-state simulation (opt-in via MASSU_PREPUSH_CLEAN=1) — P2-001
...
if [ "${MASSU_PREPUSH_CLEAN:-0}" = "1" ]; then
  echo -n "[0/20] Clean-state simulation... "
  ...
  echo "[0/20] Clean-state simulation... SKIP (MASSU_PREPUSH_CLEAN unset)"
fi

# [1/20] Node version pre-flight. massu supports Node >=20 (packages/core engines).
...
if [ -n "$NODE_VERSION" ]; then
  if [ "$NODE_VERSION" -lt 20 ]; then
    echo "[1/20] Node version pre-flight... FAIL"
    ...
    echo "[1/20] Node version pre-flight... PASS (v${NODE_VERSION}.x)"
  fi
fi

# [2/20] Pattern Scanner (~5s)
echo -n "[2/20] Pattern Scanner... "
...
# [3/20] Security Scanner (~5s)
echo -n "[3/20] Security Scanner... "
...
# [7/20] Plan Status Validator (~2s) — Plan 1.5.8 P3-002
echo -n "[7/20] Plan Status Validator... "
...
# [8/20] Plan Commit Drift (~2s) — Plan 1.5.8 P3-002
echo -n "[8/20] Plan Commit Drift... "
...
# [9/20] Deploy Staleness (~3s) — plan-1.6.3-website-feature-discoverability P-C-002
# Catches the "shipped to npm but not deployed to Vercel" structural drift class.
echo -n "[9/20] Deploy Staleness... "
...
# [10/20] Dist-Tag Pre-Release (~3s) — plan-1.7.0-cohesive-cleanup P-C-003
...
echo -n "[10/20] Dist-Tag Pre-Release... "
...
# [11/20] Public Content Leak Guard (~2s) — plan-public-content-leak-guard CR-49
...
echo -n "[11/20] Public Content Leak Guard... "
...
# [12/20] Workspace Build Freshness (~1s) — P2-005, plan-2026-05-18-pre-push-ci-parity
...
echo -n "[12/20] Workspace Build Freshness... "
if [ "${MASSU_SKIP_NEW_STEPS:-0}" = "1" ]; then
  ...
  FRESH_FAILED=0
  FRESH_REASON=""
  # Filesystem-derived workspace list (HIGH arch finding fix — same SoT as [0/20]).
  for pkg in "${WORKSPACE_PKGS[@]}"; do
...
# [13/20] Sync Check public-mirror (~30s when triggered) — P2-002
...
# [16/20] CLAUDE.md Size (~1s) — plan-2026-06-01-claude-md-size-compliance (CR-? size gate)
...
# [17/20] Coverage Gate real v8 line% (~40s) — plan-2026-06-03-website-lib-test-coverage P0-006
...
# [18/20] Plan-Token Changelog Coverage (~2s) — plan-2026-06-03-website-lib-test-coverage (CR-50 parity)
...
echo -n "[18/20] Plan-Token Changelog Coverage... "
...
# [19/20] Incident Coverage (~1s) — CR-62.
...
echo -n "[19/20] Incident Coverage (CR-62)... "
...
echo -n "[20/20] Claim Ledger (CR-63)... "
