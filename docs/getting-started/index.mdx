...
- **16 lifecycle hooks** that automatically capture observations, inject context, and enforce rules
...
Massu AI's 16 lifecycle hooks run automatically. You do not need to remember to log decisions, capture observations, or save session state. The hooks handle all of this in the background, completing in under 500ms to avoid slowing down your workflow.

### 73 Purpose-Built Tools

Massu AI provides 73 MCP tools organized across eighteen categories. Free-tier tools (12) are available to everyone; additional tools unlock with [Pro, Team, and Enterprise tiers](/docs/reference/license-tiers):
...
When you run `npx massu init`, Massu AI automatically configures the memory system, installs all 16 lifecycle hooks, and deploys 59 slash commands into `.claude/commands/`. No additional setup is required. The `session-start` hook begins injecting context from your very first session. To check memory is working:
...
Massu AI includes 59 workflow commands that enforce a verification-first approach to AI-assisted development. Every claim of completion requires proof. Every removal requires negative verification. Every plan item gets tracked.
