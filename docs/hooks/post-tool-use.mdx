...
5. **Auto-ingests memory files** -- when a Write/Edit targets a `memory/*.md` file, the YAML frontmatter (name, description, type, confidence) is parsed and the content is ingested into the observations table. This makes file-based memories queryable via `massu_memory_search` and other MCP tools.
6. **Stores the observation** in the memory database
