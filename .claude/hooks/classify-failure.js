...
init_config();
init_memory_db();
import { existsSync as existsSync8, readFileSync as readFileSync5, readdirSync as readdirSync2, unlinkSync as unlinkSync2, writeFileSync as writeFileSync3 } from "fs";
...
function getDedupeMarkerPath(sessionId, filePath) {
  ...
  return join5(tmpdir(), `massu-classify-${day}-${sessionId.slice(0, 8)}-${hash}`);
}
...
function readFailureContextFiles() {
  ...
  try {
    ...
    for (const file of files) {
      try {
        context += " " + readFileSync5(join5(dir, file), "utf-8");
      } catch {
      }
    }
  } catch {
...
async function main() {
  try {
    ...
    if (existsSync8(dedupeMarker)) {
      process.exit(0);
      return;
    }
    try {
      writeFileSync3(dedupeMarker, "1");
    } catch {
    }
    ...
    try {
      ...
      if (!bestMatch || bestMatch.score === 0) {
        outputNewPattern(basename3(filePath), bestMatch);
