...
import { existsSync as existsSync7, readFileSync as readFileSync4, unlinkSync as unlinkSync2, readdirSync as readdirSync2, statSync as statSync2 } from "fs";
...
function getSessionFlagPath(sessionId) {
  return join4(tmpdir(), "massu-auto-learning", `fixes-${sessionId.slice(0, 12)}.jsonl`);
}
async function main() {
  try {
    ...
    if (existsSync7(flagPath)) {
      try {
        sessionFixes = readFileSync4(flagPath, "utf-8").split("\n").filter(Boolean).map((line) => JSON.parse(line));
      } catch {
      }
    }
