...
function resolveModelDir() {
  try {
    ...
    let root = null;
    for (let i = 0; i < 8; i++) {
      if (existsSync4(join2(dir, "package.json"))) {
        root = dir;
        break;
      }
    ...
    for (const rel of ["dist/embedder", "assets/embedder"]) {
      const candidate = join2(root, rel);
      if (existsSync4(join2(candidate, "model_quantized.onnx"))) return candidate;
    }
    return null;
  } catch {
    return null;
...
function resolveWasmDir() {
  try {
    ...
    const main4 = req.resolve(ORT_PKG);
    return dirname3(main4);
  } catch {
    return null;
  }
}
async function getSession() {
  ...
  _sessionPromise = (async () => {
    try {
      ...
      const modelPath = join2(modelDir, "model_quantized.onnx");
      const vocabPath = join2(modelDir, "vocab.txt");
...
function getMemoryDb() {
  ...
  const preExisting = existsSync5(dbPath);
...
import { appendFileSync, mkdirSync as mkdirSync4, existsSync as existsSync6 } from "fs";
...
function resolveFailureLogPath() {
  ...
  let dir = process.cwd();
  for (let i = 0; i < 12; i++) {
    if (existsSync6(join3(dir, ".massu")) || existsSync6(join3(dir, "massu.config.yaml"))) {
...
function runSecurityGateFindings(hookInput) {
  const findings = [];
  ...
  if (tool_name === "Bash" && tool_input.command) {
    ...
    if (violation) {
      findings.push({
        ...
        message: `SECURITY GATE: Dangerous command pattern detected: ${violation.label}
Command: ${tool_input.command.slice(0, 200)}` + (violation.severity === "block" ? `
...
function getDataDb() {
  ...
  if (!existsSync7(dbPath)) return null;
  try {
...
function checkKnowledgeFileProtection(input) {
  ...
  for (const f of candidateFiles) {
    ...
    if (KNOWLEDGE_PROTECTED_FILES.includes(basename3)) {
      return `KNOWLEDGE SYSTEM PROTECTION: "${basename3}" is a core knowledge system file. Deleting it will break knowledge indexing and memory retrieval. Create a replacement before removing.`;
    }
  }
  return null;
}
...
function runPreDeleteChecks(hookInput) {
  ...
  try {
    ...
    try {
      ...
      if (!tableExists2) return messages;
...
// src/hooks/lib/read-stdin.ts
var StdinTimeoutError = class extends Error {
  bytesRead;
  constructor(ms, bytesRead) {
    super(
      `stdin did not reach EOF within ${ms}ms (${bytesRead} bytes read). The payload is INCOMPLETE and must not be treated as valid input.`
    );
    this.name = "StdinTimeoutError";
    this.bytesRead = bytesRead;
  }
};
function readStdinToEof(deadlineMs = 4e3) {
  return new Promise((resolve5, reject) => {
    ...
    process.stdin.on("end", () => finish(() => resolve5(data)));
...
async function main3() {
  ...
  try {
    ...
    deny(
      ...
      err instanceof Error ? `Cause: ${err.message}` : void 0
    );
  }
  let findings;
  try {
    ...
    const deletes = runPreDeleteChecks(
      hookInput
    ).map((message) => ({ severity: "warn", message }));
