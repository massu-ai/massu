...
function extractAlternatives(description) {
  const alternatives = [];
  ...
  if (switchMatch) {
    alternatives.push(switchMatch[2].trim());
    alternatives.push(switchMatch[1].trim());
  }
  return [...new Set(alternatives)];
}
function storeDecision(db, decision) {
  const existing = new Set(
    db.prepare(`PRAGMA table_info(architecture_decisions)`).all().map(
      (c) => c.name
    )
  );
  ...
  const extraSql = hasTemporal ? `, valid_from, ingested_at, valid_from_epoch, ingested_at_epoch` : "";
  const extraVals = hasTemporal ? `, datetime('now'), datetime('now'), CAST(strftime('%s','now') AS INTEGER), CAST(strftime('%s','now') AS INTEGER)` : "";
  const result = db.prepare(
...
function classifyRealTimeToolCall(toolName, toolInput, toolResponse, seenReads2, isError) {
  ...
  if (responseText && detectDecisionPatterns(responseText)) {
    ...
    const detail = responseText.slice(0, 1e3);
    return {
      type: "decision",
      title,
      detail,
...
init_config();
import { readFileSync as readFileSync5, existsSync as existsSync8, statSync as statSync2 } from "fs";
import { resolve as resolve6, dirname as dirname8, join as join5 } from "path";
...
function resolveImportPath(specifier, fromFile) {
  ...
  for (const indexFile of resolvedPaths.indexFiles) {
    const indexPath = join5(basePath, indexFile);
...
// src/hooks/post-tool-use.ts
init_config();
import { readFileSync as readFileSync9, writeFileSync as writeFileSync3, existsSync as existsSync12, mkdirSync as mkdirSync7, statSync as statSync3 } from "fs";
import { join as join6, basename as pathBasename2, dirname as pathDirname, resolve as pathResolve } from "path";
...
import { readFileSync as readFileSync8, existsSync as existsSync11, readdirSync as readdirSync2 } from "fs";
init_memory_db();
...
function ingestMemoryFile(db, sessionId, filePath) {
  if (!existsSync11(filePath)) return "skipped";
    ...
    try {
      ...
      if (observation.type === "decision") {
        try {
          ...
          ).get(session_id, observation.title);
          if (!existing) {
            storeDecision(db, {
      ...
      try {
        ...
        try {
          ...
          const dir = join6(projectRoot, ".massu", "rule-candidates");
          if (!existsSync12(dir)) mkdirSync7(dir, { recursive: true });
          const logPath = join6(dir, ".cr53-increment-failures.jsonl");
          const pre = existsSync12(logPath) ? readFileSync9(logPath, "utf-8") : "";
...
function readConventions(cwd) {
  try {
    ...
    const configPath = join6(projectRoot, "massu.config.yaml");
    if (!existsSync12(configPath)) return _conventionDefaults;
