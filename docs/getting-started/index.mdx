...
Massu AI is an **AI Engineering Governance Platform** that transforms how your team works with AI coding assistants. It is an MCP server and Claude Code plugin that provides cross-session memory, a structured knowledge system, automated quality tracking, compliance audit trails, and 73 specialized tools -- all running locally with zero cloud dependencies.
...
- **15 lifecycle hooks** that automatically capture observations, inject context, and enforce rules
- **73 MCP tools** for code intelligence, security scoring, cost tracking, knowledge search, and more
...
Massu AI's 15 lifecycle hooks run automatically. You do not need to remember to log decisions, capture observations, or save session state. The hooks handle all of this in the background, completing in under 500ms to avoid slowing down your workflow. The 4 auto-learning pipeline hooks go further: they detect bug fixes, enforce incident documentation, derive prevention rules, and place enforcement -- turning every fix into a project-wide safeguard.

### 72 Purpose-Built Tools

Massu AI provides 73 MCP tools organized across eighteen categories. Free-tier tools (13) are available to everyone; additional tools unlock with [Pro, Team, and Enterprise tiers](/docs/reference/license-tiers):

| Category | Tools | Tier | What They Do |
|----------|-------|------|-------------|
| **Core Intelligence** | 7 | Free | Sync indexes, get file context, analyze impact |
| **Memory & Learning** | 7 | Free (3) / Pro (4) | Search past sessions, view timelines, track failed attempts, backfill from files |
...
When you run `npx massu init`, Massu AI automatically configures the memory system, installs all 15 lifecycle hooks, and deploys 43 slash commands into `.claude/commands/`. No additional setup is required. The `session-start` hook begins injecting context from your very first session. To check memory is working:
...
Massu AI includes 43 workflow commands that enforce a verification-first approach to AI-assisted development. Every claim of completion requires proof. Every removal requires negative verification. Every plan item gets tracked.
...
| [Hooks](/docs/hooks) | The 15-hook lifecycle system |
...
Your Prompt --> Claude Code --> Massu AI Hooks --> Memory DB + Knowledge DB
                    |                                          |
                    v                                          v
              73 MCP Tools <-- Context Injection <-- Past Sessions + Knowledge
                    |
                    v
...
3. **Claude Code calls tools** -- Massu AI's 73 MCP tools provide code intelligence, memory search, knowledge lookup, quality scoring, and more
